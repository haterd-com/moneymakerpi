...
.PHONY: $(TARGETS) $(TARGETS_CONFIG) all clean help get-buildroot buildx
...
$(TARGETS): %: $(RELEASE_DIR) %-config
	...
	cp -f $(O)/images/mmos_* $(RELEASE_DIR)/
...
get-buildroot: BUILDROOT_VERSION=2021.02.7
get-buildroot:
	mkdir -p buildroot; wget -c https://buildroot.org/downloads/buildroot-${BUILDROOT_VERSION}.tar.gz -O - | tar -xz -C ./buildroot --strip-components 1

buildx: platform="linux/arm64"
buildx:
	docker --debug buildx build --platform ${platform} --load -t moneymakerpi-builder -f Dockerfile .